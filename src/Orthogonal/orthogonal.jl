...
The  command `basis(P,n, [var])` realizes the polynomial. `Basis(P,n)` does  not.  This  can  be  useful for  evaluation but *only* for small values of ``n``, as the evaluation via a hypergeometric representation is susceptible to numeric issues.
"""
struct Basis{Π}
    n::Int
end
