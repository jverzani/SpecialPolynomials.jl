...
function basis(P::Type{<:DualBernstein{𝐍,α,β}}, k::Int, _var::Polynomials.SymbolLike=:x; var=_var) where {𝐍,α,β}
    @assert k <= 𝐍
    T = eltype(P)
    zs = zeros(T, 𝐍 + 1)
    zs[k+1] = 1
...
function Base.one(P::Type{<:DualBernstein{N,α,β}}, var::Polynomials.SymbolLike=:x) where {N,α,β}
    N′ = max(N, 0)
...
function ChudyWozny_eval(p::DualBernstein{𝐍,α,β}, x) where {𝐍,α, β}
    ...
    coeffs = p.coeffs
    sum(pᵢ*Dᵢ for (pᵢ,Dᵢ) ∈ zip(coeffs, D); init=zero(x))
end
...
function AllDualBer(n, α, β, x)
    ...
    for (i,x) ∈ enumerate(fwd)
        D[i] = x
    end
    if n - J - 1 > 0
        bckwd = DualBer(n, β, α, 1-x, n- J - 1, K)
        for (i,x) ∈ enumerate(bckwd)
            D[end - i + 1] = x
        end
    end
    D
