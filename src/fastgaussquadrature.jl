...
gauss_nodes_weights(p::Type{P}, n) where {P<:Chebyshev} = gausschebyshevt(n)

gauss_nodes_weights(p::Type{P}, n) where {α,β,P<:Jacobi{α,β}} =
    FastGaussQuadrature.gaussjacobi(n, α, β)

gauss_nodes_weights(p::Type{P}, n) where {P<:Legendre} = gausslegendre(n)

gauss_nodes_weights(p::Type{P}, n) where {P<:Hermite} = gausshermite(n)

gauss_nodes_weights(p::Type{P}, n) where {α,P<:Laguerre{α}} = gausslaguerre(n, α)
